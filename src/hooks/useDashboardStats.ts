...
export const useDashboardStats = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      // Calculate monthly revenue (sum of completed projects this month)
      const currentMonth = new Date().getMonth();
      const currentYear = new Date().getFullYear();
      
      const monthlyRevenue = projectStats
        ?.filter(p => {
          if (p.status !== 'completed' || !p.price) return false;
          const projectDate = new Date(p.created_at);
          return projectDate.getMonth() === currentMonth && projectDate.getFullYear() === currentYear;
        })
        .reduce((total, p) => total + (Number(p.price) || 0), 0) || 0;
